...
static struct Rule defrules[] = {
    ...
    {NULL, "physettings", NULL, 0, 1, 0, 1, 1, 0, -1},
...
static int sendevent(Window win, Atom proto, int m, long d0, long d1, long d2,
                     long d3, long d4);
...
static const Key defkeys[] = {
    STACKKEYS(Mod4Mask, focus) STACKKEYS(Mod4Mask | ShiftMask, push)
...
static void (*handler[])(XEvent *) = {[ButtonPress] = buttonpress,
...
void autostart(void) { system("killall -q dwmblocks; dwmblocks &"); }
...
int processrawtext(char *text) {
  int x = 0, i;
  int s = strlen(text);

  for (i = 0; i < s && text[i] != '\0'; i++) {
    if (text[i] == '|') {
      /* If seperator and next char is not icon (between |0-9|, just 1
       * character) */
      if (!BETWEEN(text[i + 1], '0', '9') || i + 2 < s || text[i + 2] != '|') {
        x += (sb_delimiter_w + sb_icon_margin_x * 2) / 2;
      }
      x -= TEXTW_SB("|");
    } else if (BETWEEN(text[i], '0', '9') && text[i + 1] == '|' &&
               (i == 0 || text[i - 1] == '|')) {
  ...
  if (ev->window == root) {
    ...
    if (updategeom() || dirty) {
      ...
      for (m = mons; m; m = m->next) {
        ...
        XMoveResizeWindow(dpy, m->barwin, m->wx + sb_padding_x, m->by,
                          m->ww - 2 * sb_padding_x, bh);
      }
      focus(NULL);
      arrange(NULL);
    }
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) {
    ...
    XMoveResizeWindow(dpy, allbarwin[1],
                      m->wx + m->brightstart +
                          (showsystray ? -stw - sb_padding_x : sb_padding_x),
                      m->by, stw + twtmp - (showsystray ? 0 : 2 * sb_padding_x),
                      bh);
    if (showsystray)
      XMoveResizeWindow(dpy, systray->win, twtmp - sb_padding_x, 0,
                        (stw ? stw + sb_padding_x : 1), bh);
    ...
    while (sb_arr[i] != NULL) {
      ...
      if (strlen(sb_arr[i]) == 1) {
        ...
        drw_pic(drw, m->ww - twtmp, (bh - sb_icon_wh) / 2, sb_icon_wh,
                sb_icon_wh, None, idx);
        twtmp -= sb_icon_wh + sb_icon_margin_x;
        drw_text(drw, m->ww - twtmp, y, TEXTW_SB(sb_arr[i]), bh, 0, sb_arr[i],
                 0);
      ...
      if (sb_arr[i + 1] != NULL || showsystray) {
        ...
        drw_rect(drw, m->ww - twtmp + sb_delimiter_w / 4, y, sb_delimiter_w / 2,
                 bh, 1, 0);
        drw_rect(drw, m->ww - twtmp, y + bh / 10, sb_delimiter_w,
                 bh - 2 * bh / 10, 1, 0);
  ...
  drw_pic(drw, sb_delimiter_w, y + (bh - sb_icon_wh) / 2, sb_icon_wh,
          sb_icon_wh, None, 0);
  x += sb_icon_wh + 2 * sb_delimiter_w;
  ...
  for (i = 0; i < lenconfig; i++) {
    ...
    drw_pic(drw, x + w, (bh - sb_icon_wh) / 2, sb_icon_wh, sb_icon_wh, None,
            i + 1);
  ...
  if ((w = m->ww - tw - x - stw - 2 * sb_padding_x) > bh) {
    ...
    if (m->sel && m == selmon) {
      ...
      s = drw_text(drw, x, 0,
                   MIN(w, TEXTW(c->name) +
                              (c->icon ? c->icw + sb_icon_margin_x : 0) +
                              lrpad),
                   bh, lrpad / 2 + (c->icon ? c->icw + sb_icon_margin_x : 0),
                   c->name, HIDDEN(c) ? 1 : 0) -
          x;
      if (c->icon)
      ...
      XMoveResizeWindow(
          dpy, allbarwin[0],
          m->wx + (m->bleftend + m->brightstart - s - stw - sb_padding_x) / 2 +
              2 * sb_padding_x,
          m->by, s - 2 * sb_padding_x, bh);
      drw_map(drw, allbarwin[0], m->bleftend, 0, m->ww, bh);
    } else if (!m->sel && m == selmon) {
      XMoveWindow(dpy, allbarwin[0], m->wx + m->ww / 2,
                  -2 * (bh + sb_padding_y));
    }
  }
  ...
  if (arg->i > 0) {
    if (selmon->sel)
      for (c = selmon->sel->next;
           c && (!ISVISIBLE(c) || HIDDEN(c) || (c->issticky && c->isfloating));
           c = c->next)
        ;
    ...
    if (selmon->sel) {
      for (i = selmon->clients; i != selmon->sel; i = i->next)
        if (ISVISIBLE(i) && !HIDDEN(i) && !(i->issticky && i->isfloating))
          c = i;
    }
    if (!c)
      for (; i; i = i->next)
    ...
    for (i = 0; i < MAX(lenkeys, LENGTH(defkeys)); i++) {
      ...
      if (i < LENGTH(defkeys))
        if ((code = XKeysymToKeycode(dpy, defkeys[i].keysym)))
          for (j = 0; j < LENGTH(modifiers); j++)
            XGrabKey(dpy, code, defkeys[i].mod | modifiers[j], root, True,
                     GrabModeAsync, GrabModeAsync);
    }
  }
}
  ...
  for (i = 0; i < MAX(lenkeys, LENGTH(defkeys)); i++) {
    ...
    if (i < LENGTH(defkeys))
      if (keysym == defkeys[i].keysym &&
          CLEANMASK(defkeys[i].mod) == CLEANMASK(ev->state) && defkeys[i].func)
...
Picture geticonprop(Window win, unsigned int *picw, unsigned int *pich) {
  ...
  uint32_t w, h, sz;
  {
    ...
    for (i = p; i < end - 1; i += sz) {
      ...
      if ((sz = w * h) > end - i)
        break;
      if ((m = w > h ? w : h) >= sb_icon_wh && (d = m - sb_icon_wh) < bstd) {
        bstd = d;
        bstp = i;
      }
    }
...
void runautostart(void) {
  ...
  if (xdgconfighome != NULL && *xdgconfighome != '\0') {
    ...
    /* space for path segments, separators and nul */
    pathpfx =
        ecalloc(1, strlen(home) + strlen(localshare) + strlen(pdwmdir) + 3);
...
int sendevent(Window win, Atom proto, int m, long d0, long d1, long d2, long d3,
              long d4) {
  int n;
  Atom *protocols, mt;
  int exists = 0;
...
void showwin(Client *c) {
  ...
  XChangeProperty(dpy, c->win, netatom[NetWMName],
                  XInternAtom(dpy, "UTF8_STRING", False), 8, PropModeReplace,
                  (unsigned char *)(c->name + 8), strlen(c->name + 8));
  arrange(c->mon);
}
...
void updatebars(void) {
  ...
  for (int i = 0; i < 2; i++) {
    if (!allbarwin[i]) {
      allbarwin[i] = XCreateWindow(
          dpy, root, 0, 0, bh, bh, 0, DefaultDepth(dpy, screen), CopyFromParent,
  ...
  for (m = mons; m; m = m->next) {
    if (!m->barwin) {
      m->barwin = XCreateWindow(
          dpy, root, m->wx + sb_padding_x, m->by, m->ww - 2 * sb_padding_x, bh,
  ...
  /* Init systray */
  if (showsystray) {
    systray->win = XCreateSimpleWindow(dpy, allbarwin[1], 0, 0, 1, bh, 0, 0,
